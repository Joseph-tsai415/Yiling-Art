...
`tests/` contains Node scripts for refreshing the Pantone dataset. Run them from anywhere — they resolve paths relative to the script location.

| Script | Purpose |
|--------|---------|
| `scrape-pantone-reference.js` | Fetches pantonecolors.net and writes a comparison report against the local dataset |
| `patch-pantone-colors.js` | Applies coated-hex corrections and adds the small set of legacy special colors, writes a changelog |
| `test-color-classification.js` | Verifies every Pantone color is classified into exactly one color family |
